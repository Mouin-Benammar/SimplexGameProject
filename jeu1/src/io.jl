...
function transformer(t::Array{Int64, 2},x)
    n,m=size(t)
    res=copy(t)
     for i in 1:n
         for j in 1:m
             if abs(JuMP.value(x[i,j]))<TOL
             res[i,j]=0
         end
         end
     end
...
function  displaySolution(t::Array{Int64, 2},x)
    displayGrid(transformer(t,x))
end
